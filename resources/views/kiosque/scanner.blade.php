...
<div class="kiosque-header">
    ...
    <div class="ms-3">
        <select id="select-imprimante" class="form-select form-select-sm bg-dark text-white border-secondary" style="min-width:180px;" onchange="changerImprimante(this.value)">
            <option value="Rongta RP4xx Series" {{ session('imprimante','Rongta RP4xx Series') == 'Rongta RP4xx Series' ? 'selected' : '' }}>Rongta RP4xx</option>
            <option value="VRSTTI" {{ session('imprimante') == 'VRSTTI' ? 'selected' : '' }}>VRSTTI</option>
        </select>
    </div>
</div>
...
<script>
    window.ITA_SCANNER_URL   = "{{ route('kiosque.scanner') }}";
    window.ITA_SCANS_URL     = "{{ route('kiosque.scans-recents') }}";
    window.ITA_IMPRIMANTE_URL = "{{ route('kiosque.imprimante') }}";
